...
use crate::state::{ActionOutcome, AppliedAction, EdgeKey, GameAction, TurnState};

/// Deterministic xorshift RNG used by the baseline AI and self-play game drivers.
#[derive(Clone)]
pub struct AiRng {
    state: u64,
}
...
impl AiRng {
    /// Construct an RNG from an explicit seed (for reproducible self-play / eval).
...
pub fn choose_heuristic_action(turn_state: &TurnState, ai_rng: &mut AiRng) -> Option<GameAction> {
...
pub fn choose_alpha_beta_action(
...
fn evaluate_position(turn_state: &TurnState, root_player: usize) -> i32 {
    ...
    (best_opponent_path - root_path) * 140
        ...
        + (root_fences - opponent_fences / opponent_count.max(1)) * 4
}
