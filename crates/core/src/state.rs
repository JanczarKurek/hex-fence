...
use crate::player::{PlayerDef, fences_per_player, players_for_count};
...
impl TurnState {
    ...
    pub fn is_occupied(&self, coord: AxialCoord) -> bool {
        self.pawn_positions.contains(&coord)
    }
    ...
    /// The cheap, connectivity-independent validity checks for a fence: the current player
    /// has a fence left, the three edges are distinct, on-board, adjacent, and not already
    /// blocked. Connectivity (no player walled off) is checked separately.
    pub fn fence_geometry_valid(&self, edges: &[EdgeKey; 3]) -> bool {
    ...
    pub fn can_place_fence(&self, edges: &[EdgeKey; 3]) -> bool {
        if !self.fence_geometry_valid(edges) {
            return false;
        }
